...
resource "aws_s3_bucket_policy" "elb_log_delivery" {
  count  = var.enabled && (var.attach_elb_log_delivery_policy || var.attach_policy) ? 1 : 0
...
resource "aws_s3_bucket_policy" "encrypted_uploads" {
...
resource "aws_s3_bucket_object" "bucket_uploads" {
  ...
  etag          = filemd5("${var.object_path}/${each.value}")
  tags          = module.label.tags
}
